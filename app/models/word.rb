class Word < ApplicationRecord
  ...
  validates :content, presence: true#, if: :kara?
       ...
       # end
  validates :content, format: { with: /\A[ぁ-ゞァ-ヾ]{1,128}\z/, on: :create }, if: ->(u) { u.content.present? }#, if: :moji?
       # def moji?
  ...
  validates :content, uniqueness: true, if: :nanashi?
      def nanashi?
         ...
         # errors.add :base, "もうあるので、登録するかログインして..."
         # else
         # errors.add :base, "もうあるので、用例を書いて！⇒リンクボタン"
         # end
      end
